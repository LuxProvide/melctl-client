...
class Hinter:
    ...
    def hint_files(self, args):
        ...
        if not os.path.isfile(settings.Config.env_file):
            yield ('warning', f'Configuration file "{settings.Config.env_file}" not found')
    ...
    def hint_variables(self, args):
        ...
        # ---
        # URL format
        msg_url = ('warning', '''URL (-u, --url or config's "url") should not point to localhost or 127.0.0.1''')
        ...
        if len(args.auth) < 1 or len(settings.token) < 1:
            yield ('warning', '''Token (-a, --auth or secret "token") does not look like a valid JWT''')
    ...
    def __call__(self, args):
        ...
        """
        _hinted = False
        for hinter in [self.hint_files, self.hint_variables, ]:
            try:
                ...
                _hinted = True
                self.print_hint(args, 'problem', f'Failed to run hinter {hinter.__name__}: {str(error)}')
                if args.traceback:
                    self.print_hint(args, 'problem', 'Dumping exception as -t|--traceback is set')
                    raise
        if _hinted:
